 ...
 const handleExportScheduledTechnicians = () => {
  try {
    ...
    scheduledTechs.forEach((tech) => {
      const sch: any = schedules.find(s => s.id === tech.scheduledCertificationId);
      if (!sch) return;

      const analyst = allUsers.find(u => u.id === sch.analystId);
      const analystName = analyst?.fullName || (analyst as any)?.name || 'SEM ANALISTA';

      const company = tech.company || 'N/D';
      const cityText = `${tech.city || ''}${tech.state ? ' / ' + tech.state : ''}`;
      ...
      const publico =
        (classRef as any)?.audience ||
        (tech as any).audience ||
        'ANALISTA';
  ...
  (sch as any).theoreticalTime
    ...
    : 'N/D';
      ...
      const baseName = sch.baseName || '';
      const baseAddress = sch.baseAddress || '';
  ...
  '',
        ...
        cpf: tech.cpf ? `'${String(tech.cpf).padStart(11, '0')}` : '',
        trainingClassId: tech.trainingClassId || '',
        analystId: sch.analystId || '',
        scheduleId: sch.id || '',
        shift: sch.shift || '',
        rawType: sch.type || '',
        rawCity: tech.city || '',
        uf: tech.state || '',
        rawCompany: tech.company || '',
        baseId: sch.baseId || '',
        powerAppsBaseId: sch.powerAppsBaseId || '',
        routingRuleId: sch.routingRuleId || '',
        exportedAt
      };
    ...
    const rowsByDay: any[][] = [
      ['DATA', 'PROVA TEÓRICA', 'PROVA PRÁTICA', 'ANALISTA', 'TÉCNICO', 'TURMA', 'PÚBLICO', 'EMPRESA', 'CIDADE', 'TIPO', 'BASE', 'ENDEREÇO']
    ];
...
const handleRemoveTrainingClass = () => {
  ...
  const handleRemoveScheduledClass = () => {
  if (!selectedClassId || selectedClassId === 'GLOBAL_BACKLOG') {
    setToast({ message: 'Selecione uma turma válida para remover.', type: 'error' });
    return;
  }
  ...
  const confirmed = window.confirm(
    `Deseja remover a turma "${selectedClass.title}"?\n\nIsso irá remover os técnicos vinculados e liberar a agenda dos analistas.`
  );

  if (!confirmed) return;

  const result = dataService.removeTrainingClassFromScheduled(selectedClassId);

  if (result.success) {
    refreshData();
    setSelectedClassId(null);
    setToast({
      message: `Turma removida com sucesso. ${result.removedTechniciansCount} técnico(s) removido(s) e ${result.removedSchedulesCount} agendamento(s) limpo(s).`,
      type: 'success'
    });
  } else {
    setToast({
      message: result.message || 'Erro ao excluir turma agendada.',
      type: 'error'
    });
  }
};
...
setActiveSubTab('technicians');
  ...
  // REGRA ESPECIAL: AGENDADOS -> REPROVADOS
  if (
    isScheduledManagement &&
    withdrawType === 'REPROVADOS'
  ) {
    let outcome: 'NOSHOW' | 'REPROVADO_1_CERTIFICACAO' | 'REPROVADO_2_CERTIFICACAO';

    if (withdrawSubReason === 'NOSHOW') {
      outcome = 'NOSHOW';
    } else if (withdrawSubReason === 'REPROVADO 1º CERTIFICAÇÃO') {
      outcome = 'REPROVADO_1_CERTIFICACAO';
    } else if (withdrawSubReason === 'REPROVADO 2º CERTIFICAÇÃO') {
      const confirmed = window.confirm(
        'Este técnico será enviado para INABILITADOS e precisará refazer o treinamento. Deseja continuar?'
      );
      if (!confirmed) return;
      outcome = 'REPROVADO_2_CERTIFICACAO';
    } else {
      setToast({ message: 'Submotivo inválido para técnico agendado.', type: 'error' });
      return;
    }
    ...
    if (res.success) {
      if ((res as any).autoPromotedToSecondFailure) {
        setToast({
          message: 'O sistema identificou tentativa anterior e tratou como 2ª certificação. Técnico enviado para INABILITADOS.',
          type: 'success'
        });
      } else if ((res as any).movedToIneligible) {
        setToast({
          message: 'Técnico enviado para INABILITADOS.',
          type: 'success'
        });
      } else {
        setToast({
          message: `Atualizado: REPROVADOS – ${withdrawSubReason}`,
          type: 'success'
        });
      }
...
setWithdrawObservation('');
      ...
      setToast({
        message: (res as any).message || 'Falha ao atualizar técnico.',
        type: 'error'
      });
    }
  ...
  // FLUXO ATUAL - FILA E DEMAIS SITUAÇÕES
  let targetStatus = '';
  let category = '';
...
const isScheduledManagement =
  !!selectedTechnician &&
  (
    selectedTechnician.status_principal === 'AGENDADOS' ||
    selectedTechnician.certificationProcessStatus === CertificationProcessStatus.SCHEDULED
  );

const subReasons = useMemo(() => {
  if (withdrawType === 'REPROVADOS') {
    if (isScheduledManagement) {
      return [
        'NOSHOW',
        'REPROVADO 1º CERTIFICAÇÃO',
        'REPROVADO 2º CERTIFICAÇÃO'
      ];
    }
    ...
    return [
      ...
      'REPROVADO EAD',
      'REPROVADO VIRTUAL',
      'REPROVADO CERTIFICAÇÃO'
    ];
  }

  if (withdrawType === 'NÃO REALIZAR') {
    return ['GESTOR', 'TREINAMENTO NÃO NECESSITA'];
  }

  if (withdrawType === 'AGENDAR POSTERIORMENTE') {
    return ['SOLICITADO PELO GESTOR', 'ANALISTA INDISPONÍVEL'];
  }

  return [];
}, [withdrawType, isScheduledManagement]);
...
const practicalTime = getScheduledExportTime(tech);
...
<div className="space-y-1.5">
  ...
  <div className="flex flex-wrap gap-4 pt-1">
    {['ANALISTA', 'MULTIPLICADOR'].map(aud => (
      <label key={aud} className="flex items-center gap-2 cursor-pointer">
        <input
          type="radio"
          name="audience"
          className="accent-claro-red"
          checked={formClass.audience === aud}
          onChange={() =>
            setFormClass({
              ...formClass,
              audience: aud as 'ANALISTA' | 'MULTIPLICADOR'
            })
          }
        />
        <span className="text-[10px] font-black uppercase">{aud}</span>
      </label>
    ))}
  </div>
</div>
...
<div className="space-y-1.5">
  ...
  <div className="grid grid-cols-2 gap-4">
    ...
    <div>
      <p className="text-[8px] font-black text-slate-400 uppercase">Público</p>
...
<div className="text-center py-10 border-4 border-dashed border-slate-100 rounded-[40px] hover:border-claro-red transition-all cursor-pointer group" onClick={openFilePickerTecnicos}>
  ...
  <div>
    <p className="text-[9px] font-black text-slate-400 uppercase">Público</p>
    <p className="text-[10px] font-black text-slate-900 uppercase">
      {(lastClassCreated as any).audience || 'ANALISTA'}
    </p>
  </div>
</div>
               ...
               <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1.5"><label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Tipo de Ação</label>
                      <select
  className="w-full bg-white border-2 border-slate-100 rounded-2xl px-5 py-3 text-xs font-bold uppercase outline-none focus:border-claro-red transition-all"
  value={withdrawType}
  onChange={(e) => {
...
>
  ...
  <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
    ...
    <div className="space-y-1.5">
      ...
      <select
        className="w-full bg-white border-2 border-slate-100 rounded-2xl px-5 py-3 text-xs font-bold uppercase outline-none focus:border-claro-red"
        value={withdrawSubReason}
        onChange={(e) => setWithdrawSubReason(e.target.value)}
      >
    ...
    {/* 🔥 AVISO INTELIGENTE - 1ª vira 2ª */}
    {withdrawType === 'REPROVADOS' &&
      isScheduledManagement &&
      withdrawSubReason === 'REPROVADO 1º CERTIFICAÇÃO' &&
      (selectedTechnician?.certificationReproofCount || 0) >= 1 && (
        <div className="bg-amber-50 p-4 rounded-2xl border border-amber-200 flex items-center gap-3">
          <span className="text-lg">⚠️</span>
          <p className="text-[10px] font-black text-amber-700 uppercase tracking-tight leading-tight">
            Este técnico já possui tentativa anterior registrada. 
            O sistema tratará esta ação como 2ª tentativa e enviará o técnico para INABILITADOS.
          </p>
        </div>
      )}

    {/* 🔥 AVISO DIRETO - 2ª reprovação */}
    {withdrawType === 'REPROVADOS' &&
      isScheduledManagement &&
      withdrawSubReason === 'REPROVADO 2º CERTIFICAÇÃO' && (
        <div className="bg-rose-50 p-4 rounded-2xl border border-rose-200 flex items-center gap-3">
          <span className="text-lg">⛔</span>
          <p className="text-[10px] font-black text-rose-700 uppercase tracking-tight leading-tight">
            Esta ação enviará o técnico para INABILITADOS e será necessário refazer o treinamento.
          </p>
        </div>
      )}

    {/* 🔥 AVISO EXTRA - NOSHOW acumulado */}
    {withdrawType === 'REPROVADOS' &&
      isScheduledManagement &&
      withdrawSubReason === 'NOSHOW' &&
      (selectedTechnician?.certificationReproofCount || 0) >= 1 && (
        <div className="bg-amber-50 p-4 rounded-2xl border border-amber-200 flex items-center gap-3">
          <span className="text-lg">⚠️</span>
          <p className="text-[10px] font-black text-amber-700 uppercase tracking-tight leading-tight">
            Este técnico já possui tentativa anterior. 
            Este NOSHOW será tratado como 2ª tentativa e poderá gerar INABILITAÇÃO automática.
          </p>
        </div>
      )}
...
setWithdrawObservation('');
                ...
                className="flex-1 py-4 text-xs font-black text-slate-400 uppercase tracking-widest">Cancelar</button><button disabled={!withdrawType || !withdrawSubReason} onClick={handleWithdrawConfirm} className="flex-1 py-4 bg-slate-900 text-white text-xs font-black uppercase rounded-2xl shadow-xl disabled:opacity-20 tracking-widest transition-all">Confirmar</button></div>
          </div>
        </div>
      )}
