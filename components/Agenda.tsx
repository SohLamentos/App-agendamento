...
const Agenda: React.FC<AgendaProps> = ({ user }) => {
  const analysts = useMemo(() => {
    return dataService.getUsers().filter(
      (u) =>
  ...
  const getCellContent = (userId: string, dateIso: string) => {
    ...
    const daySchs = schedules.filter(
      (s) =>
        s.analystId === userId &&
        ...
        <>
          ...
          >
            <div className="flex flex-col">
              ...
              <button
                onClick={() => setStatus('FOLGA')}
                className="w-full text-left px-8 py-4 text-[11px] font-black text-slate-600 hover:bg-slate-600 hover:text-white uppercase transition-all tracking-wider"
              >
                Lançar Folga
              </button>
              <button
