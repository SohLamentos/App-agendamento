...
const Agenda: React.FC<AgendaProps> = ({ user }) => {
  ...
  const [schedules, setSchedules] = useState<CertificationSchedule[]>(dataService.getSchedules() || []);
  ...
  const [outrosColor, setOutrosColor] = useState(OUTROS_PALETTE[6].color);
  ...
  const getCellContent = (userId: string, dateIso: string) => {
    ...
    const dayBlocks = safeEvents.filter(
      (e) => e.involvedUserIds?.includes(userId) && e.startDatetime?.startsWith(dateIso)
    );
    const daySchs = safeSchedules.filter(
      (s) =>
        s.analystId === userId &&
    ...
    if (fullDayBlock) {
      ...
      else if (title.includes('OUTROS')) color = fullDayBlock.color || COLORS.OUTROS;
      else if (title.includes('TREINAMENTO')) color = COLORS.TREINAMENTO;

      const displayTitle = title.includes('OUTROS - ')
    ...
    return (
      <div className="flex flex-col h-full w-full overflow-hidden">
        <div className="flex-1 flex overflow-hidden border-b border-white/20">
          {morningBlock
            ? renderCard(
                (morningBlock.title || '').replace('OUTROS - ', ''),
                morningBlock.color ||
                  ((morningBlock.title || '').includes('FÉRIAS')
                    ? COLORS.FERIAS
                    : (morningBlock.title || '').includes('FOLGA')
                    ? COLORS.FOLGA
                    : (morningBlock.title || '').includes('OUTROS')
                    ? COLORS.OUTROS
                    : COLORS.BLOQUEIO)
              )
            ...
            ? renderCard(
                ...
                morningSchs[0].type === ExpertiseType.VIRTUAL
                  ? COLORS.VIRTUAL
                  : COLORS.PRESENTIAL
              )
            : null}
        ...
        <div className="flex-1 flex overflow-hidden">
          {afternoonBlock
            ? renderCard(
                (afternoonBlock.title || '').replace('OUTROS - ', ''),
                afternoonBlock.color ||
                  ((afternoonBlock.title || '').includes('FÉRIAS')
                    ? COLORS.FERIAS
                    : (afternoonBlock.title || '').includes('FOLGA')
                    ? COLORS.FOLGA
                    : (afternoonBlock.title || '').includes('OUTROS')
                    ? COLORS.OUTROS
                    : COLORS.BLOQUEIO)
              )
            ...
            ? renderCard(
                ...
                afternoonSchs[0].type === ExpertiseType.VIRTUAL
                  ? COLORS.VIRTUAL
                  : COLORS.PRESENTIAL
              )
            : null}
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        setToast({
          message: `${inserted} agendamentos de teste importados!`,
          type: 'success'
        });
      } catch (err: any) {
  ...
  const checkImprovisoShift = (shift: Shift) => {
    if (!selection) return;
    const count = dataService.getSchedulesImpactedByImproviso(
      selection.userId,
      selection.dateIso,
      shift
    ).length;
  ...
  return (
    <div className="flex flex-col space-y-6 h-full relative">
      {toast && (
        <div
          className={`fixed top-10 right-10 z-[300] px-8 py-4 rounded-2xl shadow-2xl font-black text-xs uppercase tracking-widest animate-in slide-in-from-right-10 duration-300 ${
            toast.type === 'success'
        ...
        <div className="flex items-center space-x-4">
          <div className="flex bg-slate-50 border-2 border-slate-100 rounded-2xl overflow-hidden shadow-sm">
            <button
              ...
              className="p-3 hover:bg-slate-200 border-r border-slate-100"
            >
            ...
            <div className="px-8 py-3 text-[10px] font-black text-slate-900 uppercase min-w-[220px] text-center tracking-widest">
              {weekDates[0].iso.split('-').reverse().slice(0, 2).join('/')} —{' '}
              {weekDates[4].iso.split('-').reverse().slice(0, 2).join('/')}
            </div>

            <button
              onClick={() => navigateWeek(1)}
              className="p-3 hover:bg-slate-200 border-l border-slate-100"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-center gap-3 bg-amber-50 px-4 py-2.5 rounded-2xl border-2 border-amber-100">
              ...
              <button
                ...
                className={`w-12 h-6 rounded-full relative transition-all ${
                  isTestMode ? 'bg-amber-500' : 'bg-slate-300'
                }`}
              >
                <div
                  className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${
                    isTestMode ? 'left-7' : 'left-1'
                  }`}
                ></div>
              </button>
            </div>
      ...
      <div
        className={`bg-white border-2 rounded-[40px] shadow-sm overflow-auto flex-1 relative no-scrollbar transition-colors ${
          isTestMode ? 'border-amber-400 bg-amber-50/20' : 'border-slate-200'
        }`}
      >
        {isTestMode && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[50] bg-amber-500 text-white px-6 py-2 rounded-full font-black text-[10px] uppercase tracking-[0.3em] shadow-xl">
            Visualizando Ambiente de Teste
          </div>
        )}
        ...
        <table className="w-full border-collapse table-fixed min-w-[1400px]">
          <thead>
            <tr className="bg-slate-900 text-white shadow-xl">
              <th className="w-72 p-4 text-left font-black text-[11px] border-r-2 border-white/20 sticky top-0 z-40 bg-slate-900 uppercase tracking-widest">
                Equipe Analistas
              </th>
              {weekDates.map((d, idx) => (
                <th
          ...
          <tbody>
            {analysts.map((analyst, aIdx) => (
              <React.Fragment key={analyst.id}>
                <tr
                  className={`${
                    aIdx % 2 === 0 ? 'bg-white' : 'bg-[#f5f7fa]'
                  } border-b border-slate-900/10 h-24 transition-colors`}
                >
                  <td className="p-0 border-r-2 border-slate-300 bg-inherit shadow-md h-24">
                    <div className="flex items-center p-4 h-full">
                      <span
                        className={`w-1.5 h-8 mr-4 rounded-full ${
                          aIdx % 2 === 0 ? 'bg-claro-red' : 'bg-slate-900'
                        }`}
                      ></span>
                ...
                <tr>
                  <td colSpan={6} className="h-1 bg-slate-900/10"></td>
                </tr>
              </React.Fragment>
            ))}
          </tbody>
      ...
      <div className="flex gap-4 flex-wrap px-4 pb-4">
        ...
        <div className="flex items-center gap-1.5 text-[9px] font-black tracking-widest uppercase">
          <div className="flex gap-0.5">
            {OUTROS_PALETTE.slice(0, 3).map((p) => (
              <div
                key={p.id}
      ...
      {selection && (
        <>
          ...
          <div
            className="fixed z-[70] bg-white border border-slate-200 shadow-2xl rounded-[32px] py-6 w-72 animate-in zoom-in duration-200"
            style={{
              top:
                selection.rect.bottom + 12 > window.innerHeight - 350
                  ? selection.rect.top - 350
          ...
          >
            <div className="flex flex-col">
              ...
              <button onClick={() => setStatus('FÉRIAS')} className="w-full text-left px-8 py-4 text-[11px] font-black text-claro-red hover:bg-claro-red hover:text-white uppercase transition-all tracking-wider">
                Lançar Férias
              </button>
              <button onClick={() => setStatus('FOLGA')} className="w-full text-left px-8 py-4 text-[11px] font-black text-slate-600 hover:bg-slate-600 hover:text-white uppercase transition-all tracking-wider">
                Lançar Folga
              </button>
              <button onClick={() => setStatus('IMPREVISTO')} className="w-full text-left px-8 py-4 text-[11px] font-black text-[#6A1B9A] hover:bg-[#6A1B9A] hover:text-white uppercase transition-all tracking-wider">
                Lançar Improviso
              </button>
              <button onClick={() => setStatus('TREINAMENTO')} className="w-full text-left px-8 py-4 text-[11px] font-black text-slate-900 hover:bg-slate-100 uppercase transition-all tracking-wider">
                Treinamento
              </button>
              <button onClick={() => setStatus('OUTROS')} className="w-full text-left px-8 py-4 text-[11px] font-black text-[#455A64] hover:bg-[#455A64] hover:text-white uppercase transition-all tracking-wider">
                Outros (Motivo)
              </button>
              <button onClick={() => setStatus(null)} className="w-full text-left px-8 py-4 text-[11px] font-black text-slate-400 hover:bg-slate-50 uppercase transition-all tracking-wider mt-2 border-t border-slate-100 italic">
                Limpar Célula
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-sm overflow-hidden border-t-8 border-[#6A1B9A] animate-in zoom-in duration-300">
            <div className="bg-[#6A1B9A] p-8 text-white text-center">
              ...
              <p className="text-[10px] font-bold uppercase mt-1 opacity-70">
                Define indisponibilidade imediata
              </p>
            </div>
            ...
            <div className="p-8 space-y-4">
              <div className="flex flex-col gap-2">
                <button
                  onClick={() => checkImprovisoShift(Shift.MORNING)}
                  className={`w-full p-4 rounded-2xl border-2 font-black text-[11px] uppercase transition-all ${
                    improvisoShift === Shift.MORNING
                ...
                <button
                  onClick={() => checkImprovisoShift(Shift.AFTERNOON)}
                  className={`w-full p-4 rounded-2xl border-2 font-black text-[11px] uppercase transition-all ${
                    improvisoShift === Shift.AFTERNOON
                      ? 'border-[#6A1B9A] bg-[#6A1B9A]/10 text-[#6A1B9A]'
                      : 'border-slate-100 text-slate-400'
                  }`}
                >
                ...
                <button
                  onClick={() => checkImprovisoShift(Shift.FULL_DAY)}
                  className={`w-full p-4 rounded-2xl border-2 font-black text-[11px] uppercase transition-all ${
                    improvisoShift === Shift.FULL_DAY
                      ? 'border-[#6A1B9A] bg-[#6A1B9A]/10 text-[#6A1B9A]'
                      : 'border-slate-100 text-slate-400'
                  }`}
                >
              ...
              {impactCount > 0 && (
                <div className="bg-rose-50 p-4 rounded-2xl border border-rose-100 animate-in shake duration-500">
                  <p className="text-[10px] font-black text-rose-600 uppercase leading-tight tracking-tighter">
                    ⚠️ Este imprevisto irá cancelar {impactCount} agendamentos já
                    existentes e mover os técnicos para CANCELADOS (ANALISTA). Deseja
                    continuar?
                  </p>
                </div>
              )}
            ...
            <div className="flex gap-4 p-8 pt-0">
              <button
                onClick={() => setIsImprovisoModal(false)}
                className="flex-1 py-4 text-xs font-black text-slate-400 uppercase tracking-widest"
              >
                Voltar
              </button>
              <button
                onClick={() => setStatus('IMPREVISTO', improvisoShift)}
                className="flex-1 py-4 bg-[#6A1B9A] text-white text-xs font-black uppercase rounded-2xl shadow-xl tracking-widest"
              >
                Confirmar
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/70 backdrop-blur-md p-4">
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-sm overflow-hidden border-t-8 border-slate-900 animate-in zoom-in duration-300">
            <div className="bg-slate-900 p-8 text-white text-center">
              ...
              <p className="text-[10px] font-bold uppercase mt-1 opacity-70">
                Descrição livre e escolha de cor
              </p>
            </div>
            ...
            <div className="flex gap-4 p-8 pt-0">
              <button
                onClick={() => setIsOutrosModalOpen(false)}
                className="flex-1 py-4 text-xs font-black text-slate-400 uppercase tracking-widest"
              >
                Voltar
              </button>
              <button
                onClick={() => setStatus('OUTROS', Shift.FULL_DAY, outrosColor)}
                className="flex-1 py-4 bg-slate-900 text-white text-xs font-black uppercase rounded-2xl shadow-xl hover:bg-black transition-colors tracking-widest"
              >
                Gravar
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/70 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-md overflow-hidden border-t-8 border-claro-red animate-in zoom-in duration-300">
            <div className="bg-claro-red p-10 text-white">
              ...
              <p className="text-[10px] font-bold text-white/70 uppercase mt-1 tracking-widest">
                Escala de Indisponibilidade
              </p>
            </div>

            <div className="p-10 space-y-6">
              <select
                className="w-full border-2 border-slate-100 bg-slate-50 rounded-2xl p-4 text-xs font-black uppercase outline-none focus:border-claro-red"
                value={rangeAnalystId}
                onChange={(e) => setRangeAnalystId(e.target.value)}
              >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    type="date"
                    className="w-full border-2 border-slate-100 bg-slate-50 rounded-2xl p-4 text-xs font-black"
                    value={rangeStart}
            ...
            <div className="flex gap-4 p-10 pt-0">
              ...
              <button
                onClick={() => {
                  ...
                  setToast({ message: 'Lote gravado com sucesso!', type: 'success' });
                }}
                disabled={!rangeAnalystId || !rangeStart || !rangeEnd}
                className="flex-1 py-4 bg-claro-red text-white text-xs font-black uppercase rounded-2xl shadow-xl disabled:opacity-30 tracking-widest"
              >
                Gravar Lote
              </button>
            </div>
